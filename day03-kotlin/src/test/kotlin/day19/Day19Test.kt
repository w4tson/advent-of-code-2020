...
//        println(s)
        if (s.contains("\"")) {
            ...
            return And(index.toInt(), listOf(rule1.toInt()))
        } else if (singleEther.matches(s)) {
            val (index, rule1, rule2) = singleEther.find(s)!!.destructured
            return Or(index.toInt(), listOf(rule1.toInt()), listOf(rule2.toInt()))
        } else if (triple.matches(s)) {
            val (index, rule1, rule2, rule3) = triple.find(s)!!.destructured
            return And(index.toInt(), listOf(rule1.toInt(), rule2.toInt(), rule3.toInt()))
        } else  {
            val match = r.find(s)!!
            val (index, rule1, rule2, rule3, rule4) = match.destructured
            if (rule3 == "") {
                return And(index.toInt(), listOf(rule1.toInt(), rule2.toInt()))
            } else {
                return Or(index.toInt(), listOf(rule1.toInt(), rule2.toInt()), listOf(rule3.toInt(), rule4.toInt()))
            }
        }
    }
